...
Mesh* ResourceManager::GetLineMesh()
{
	auto& inst = Instance();
	if (!inst.lineMesh)
	{
		inst.lineMesh = CreateLineMesh();
	}
	return inst.lineMesh;
}

PipelineState* ResourceManager::GetLinePSO() 
{
	auto& inst = Instance();
	if (!inst.linePSO)
	{
		inst.linePSO = new PipelineState();
		inst.linePSO->Init(
			DeviceManager::Instance().GetDevice(),
			RootSignature::Instance().Get(),
...
Mesh* ResourceManager::CreateLineMesh() 
{
